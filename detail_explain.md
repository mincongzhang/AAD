...
It's the time we introduce the __dual number__. It's similar to the concept of complex numbers, where we have the equation $x^2 + 1 = 0$ and we define the imaginary number $i$ with the property $i^2 = -1$. We extend the real numbers by adding a new variable, $d$, with the property that $d^2 = 0$
